...
describe("ERC20 From Scratch", () =>{
    ...
    beforeEach(async () =>{
        ...
        OZERC20 = await ethers.deployContract("TokenFromScratch",["Dojo Coin", "DJC", 1000000])
    ...
    describe("Constructor and Deployment", () =>{
        ...
        it("checks name, symbol, total supply as Dojo Coin, DJC, 1,000,000", async () =>{
            ...
            expect(await OZERC20.totalSupply()).to.equal(totalSupply)
        })
    ...
    describe("Sending tokens", () =>{
        ...
        describe("Failure", () =>{
            it('rejects insufficient balances', async () => {
                ...
                await expect(OZERC20.connect(deployer).transfer(user1.address, invalidAmount)).to.be.revertedWith("Low balance")
            })
            it('rejects invalid receiver', async () => {
                const amount = tokens(100)
                await expect(OZERC20.connect(deployer).transfer('0x0000000000000000000000000000000000000000', amount)).to.be.revertedWith("Cannot send to null address")
            })
        })
    })
    describe("Approving Tokens", ()=>{
        ...
        describe("Failure", () =>{
            ...
            it("should fail the transfer of an amount > total supply", async () =>{
                await expect(OZERC20.connect(exchange).transferFrom(deployer.address, user1.address, invalidAmount)).to.be.revertedWith("Low balance")
            })
        })
    })
})